# Prompt T3 — Teste E2E Fluxo Completo de Venda (Claude CLI)
...
123 testes passando (59 unit + 10 RLS + 54 controller).
Este prompt implementa o fluxo E2E de venda — o caminho crítico do negócio.

## Padrões obrigatórios (aprendidos em T1/T2)

1. **Container:** Singleton pattern via `static {}` — NÃO usar `@Testcontainers/@Container`
2. **Herança:** estender `PostgresIntegrationSupport` — NÃO duplicar configuração
3. **Setup:** usar `JdbcTemplate` puro com `SET app.current_tenant` antes de qualquer INSERT
4. **Isolamento:** `@BeforeEach` limpa estado via SQL direto (não via JPA)
5. **UUID fixo:** `USER_ID = 00000000-0000-0000-0001-000000000001` (criado pelo DataInitializer)
6. **ON CONFLICT:** todo INSERT de setup usa `ON CONFLICT DO NOTHING`
7. **Tenant dev:** `00000000-0000-0000-0000-000000000001` (criado pelo DataInitializer)

---

## Tarefa — Criar SaleFlowIT.java

**Arquivo:** `backend/src/test/java/com/dipdv/e2e/SaleFlowIT.java`
...
# Rodar apenas o E2E
mvn test -Dtest=SaleFlowIT -Dexclude.integration.tests= 2>&1 | \
...
Meta: `SaleFlowIT: Tests run: 4, Failures: 0, Errors: 0`
Meta final: `Tests run: 58, Failures: 0, Errors: 0`
...
```
## Implementado
- e2e/SaleFlowIT.java (criado — 4 testes)

## Testes
58 testes integração, 0 falhas — BUILD SUCCESS
